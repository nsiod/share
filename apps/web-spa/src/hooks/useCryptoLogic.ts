...
export const useCryptoLogic = ({
          ...
          try {
            ...
            if (metadata.encryptionType === 'pubk') {
              if (state.processMode !== 'decrypt') {
                ...
                toast.info(
                  'Detected public key encrypted text, switching to decrypt mode',
                )
              }
              ...
              toast.error(
                'Password encrypted content is not supported in this mode',
              )
              updateState({ textInput: '' })
              return
            } else {
              if (state.processMode !== 'encrypt') {
                ...
                toast.info(
                  'Detected unencrypted text, switching to encrypt mode',
                )
              }
            }
          } catch (error) {
      ...
      try {
        ...
        if (metadata.encryptionType === 'pubk') {
          if (state.inputType !== 'file') {
            toast.info(
              'Detected public key encrypted file, switching to Key Encryption mode',
            )
          }
          updateState({
  ...
  const handleDownload = useCallback(() => {
    if (state.encryptedData) {
      ...
      toast.success(
        `${state.processMode === 'encrypt' ? 'Encrypted' : 'Decrypted'} ${state.inputType === 'file' ? 'file' : 'text'} downloaded successfully`,
      )
    }
  ...
  const processInput = useCallback(async () => {
    ...
    if (!state.keyInput) {
      toast.error(
        `Please enter a ${state.processMode === 'encrypt' ? 'public key' : 'private key or mnemonic'}`,
      )
      return
    }
    ...
    try {
      ...
      if (mode === 'encrypt') {
        if (isBase58String(_keyInput)) {
          ...
          throw new Error(
            'Invalid input. Please enter a valid Base58 public key',
          )
        }
      } else {
        if (isHexString(_keyInput)) {
          ...
          throw new Error(
            'Invalid input. Please enter a valid private key (64 hex characters) or mnemonic phrase',
          )
        }
      }
