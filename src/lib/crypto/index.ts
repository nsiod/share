...
const CONFIG = {
  ...
  ARGON2: {
    t: 3, // Time cost
    m: 65536, // Memory cost (64 MiB) - OWASP recommends >= 19 MiB
    p: 4, // Parallelism
  ...
  return new Promise((resolve, reject) => {
    ...
    const checkMemory = () => {
      ...
      if (usage === 0 || usage < threshold) {
        ...
        reject(
          new Error('Memory pressure timeout: insufficient memory available'),
        )
      } else {
...
class StreamCipher {
  ...
  async encryptChunk(chunk: Uint8Array): Promise<Uint8Array> {
    try {
      ...
      const cipher = managedNonce(xchacha20poly1305)(this.key)
...
function createStreamHeader(streamHeader: StreamHeader) {
  ...
  try {
    if (streamHeader.pwd) {
      const aes = managedNonce(xchacha20poly1305)(streamHeader.pwd.key)
    ...
    if (streamHeader.key) {
      ...
      const headerJson = JSON.stringify(header)
      const headerBytes = utf8ToBytes(headerJson)
      const aes = managedNonce(xchacha20poly1305)(streamHeader.key.key)
...
export function parseStreamHeader(
  ...
  try {
    ...
    if (isPasswordMode) {
      ...
      const aes = managedNonce(xchacha20poly1305)(key)
      ...
      const aes = managedNonce(xchacha20poly1305)(symmetricKey)
      const headerBytes = aes.decrypt(encryptedHeaderBytes)
      const headerJson = bytesToUtf8(headerBytes)
      const header = JSON.parse(headerJson) as HeaderData
...
export async function streamDecryptWithPrivateKey(
  ...
  try {
    ...
    // Verify signature against decrypted plaintext (must match what was signed during encryption)
    let isValid: boolean | undefined
    if (signature && sender) {
      onStage?.('Verifying digital signature...')
      const decryptedBlob = new Blob(chunks as BlobPart[])
      const decryptedFile = new File([decryptedBlob], 'decrypted')
      const plaintextHash = await hashFile(decryptedFile)
      isValid = secp256k1.verify(signature, plaintextHash, sender, {
        prehash: false,
      })

      if (!isValid) {
        throw new DecryptionError(ERROR_MESSAGES.SIGNATURE_VERIFY_FAILED)
      }
    }
